...
const locatePasteld = () => {
  ...
  // con.log(`Unified = ${path.join(remote.app.getAppPath(), '..', 'bin', 'mac', 'pasteld')}`);

  if (os.platform() === 'darwin') {
    return path.join(remote.app.getAppPath(), '..', 'bin', 'mac', 'pasteld');
  }

  if (os.platform() === 'linux') {
    return path.join(remote.app.getAppPath(), '..', 'bin', 'linux', 'pasteld');
  }

  return path.join(remote.app.getAppPath(), '..', 'bin', 'win', 'pasteld.exe');
};
...
class LoadingScreen extends Component<Props, LoadingScreenState> {
  ...
  async loadPastelConf(createIfMissing: boolean) {
    // Load the RPC config from pastel.conf file
    const pastelLocation = locatePastelConf();
    let confValues;
    try {
      confValues = ini.parse(await fs.promises.readFile(pastelLocation, { encoding: 'utf-8' }));
    } catch (err) {
      if (createIfMissing) {
    ...
    if (!rpcConfig.username || !rpcConfig.password) {
      this.setState({
        currentStatus: (
          <div>
            <p>Your pastel.conf is missing a &quot;rpcuser&quot; or &quot;rpcpassword&quot;.</p>
            <p>
              Please add a &quot;rpcuser=some_username&quot; and &quot;rpcpassword=some_password&quot; to your
              pastel.conf to enable RPC access
            </p>
  ...
  createPastelconf = async () => {
    ...
    this.setState({ creatingPastelConf: false });
  ...
  setupExitHandler = () => {
    ...
    ipcRenderer.on('appquitting', async () => {
      if (this.pasteld) {
        ...
        this.setState({ currentStatus: 'Waiting for pasteld to exit...' });
  ...
  startPasteld = async () => {
    ...
    if (pasteldSpawned) {
      this.setState({ currentStatus: 'pasteld start failed' });
      return;
    }
    ...
    this.setState({ pasteldSpawned: 1 });
    this.setState({ currentStatus: 'pasteld starting...' });

    this.pasteld.on('error', err => {
      console.log(`pasteld start error, giving up. Error: ${err}`);
      // Set that we tried to start pasteld, and failed
      this.setState({ pasteldSpawned: 1 });
  ...
  async getInfo() {
    const { rpcConfig, pasteldSpawned, getinfoRetryCount } = this.state;
    ...
    try {
      ...
      if (err === NO_CONNECTION && pasteldSpawned && getinfoRetryCount < 10) {
        this.setState({ currentStatus: 'Waiting for pasteld to start...' });
      ...
      if (err === NO_CONNECTION && pasteldSpawned && getinfoRetryCount >= 10) {
        ...
        this.setState({
          currentStatus: (
            <span>
              Failed to start pasteld. Giving up! Please look at the debug.log file.
              <br />
              <span className={cstyles.highlight}>{`${locatePastelConfDir()}/debug.log`}</span>
              <br />
  ...
  render() {
    const { loadingDone, currentStatus, creatingPastelConf, connectOverTor, enableFastSync } = this.state;
          ...
          {creatingPastelConf && (
            <div>
              <div className={cstyles.verticalflex}>
                ...
                >
                  <div className={[cstyles.xlarge].join(' ')}> Welcome To Pastel Wallet Fullnode!</div>
                </div>
                ...
                <div className={cstyles.left} style={{ width: '75%', marginLeft: '15%' }}>
                  ...
                  <div className={[cstyles.verticalflex].join(' ')}>
                    ...
                    <div className={cstyles.sublight}>
                      When enabled, Pastel Wallet will skip some expensive verifications of the pasteld blockchain when
                ...
                <div className={cstyles.buttoncontainer}>
                  <button type="button" className={cstyles.primarybutton} onClick={this.createPastelconf}>
                    Start Pastel
                  </button>
                </div>
              </div>
