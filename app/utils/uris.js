...
export const parsePastelURI = (uri: string): PastelURITarget[] | string => {
  if (!uri || uri === '') {
    return 'Bad URI';
  }
  ...
  const targets: Map<number, PastelURITarget> = new Map();
  ...
  const ans: PastelURITarget[] = new Array(targets.size);
