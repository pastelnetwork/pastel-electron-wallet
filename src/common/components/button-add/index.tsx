...
const ButtonAdd: React.FC<ButtonAddProps> = ({ onClick, className }) => {
  const classes = cn(className, {
    'button-add transition duration-300 border-2 flex items-center justify-center border-button-default w-7 h-7 rounded-full focus:outline-none hover:border-button-hover group': true,
  })

  return (
    <button onClick={onClick} className={classes}>
      <svg
        className='transition duration-300 button-add-icon fill-button-default group-hover:fill-button-hover'
        width='12'
        height='12'
        viewBox='0 0 12 12'
        fill='none'
