...
const createElement = (root: HTMLElement) => {
...
const setSelectionRange = (input: any) => {
...
const renderer = (tickrate: any, onrender: any) => {
  let lastTick = 0
...
const printer = (el: any, buflen: any) => (buffer: any) => {
  if (buffer.length > 0) {
...
// Parses input
const parser = (onparsed: any) => (str: string) => {
  if (str.length) {
...
// Command executor
const executor = (commands: any) => (cmd: any, ...args: any[]) => (cb: any) => {
  try {
...
const keyboard = (parse: any) => {
  let input: any[] = []
  const keys: any = { 8: 'backspace', 13: 'enter' }
  const ignoreKey = (code: number) => code >= 33 && code <= 40
  const key = (ev: any) => keys[ev.which || ev.keyCode]
...
const terminal = (opts: any) => {
  let buffer: any[] = [] // What will be output to display
  let busy = false // If we cannot type at the moment

  const { prompt, banner, commands, buflen, tickrate } = createOptions(opts)
  const $root: any = document.querySelector('#terminal')
  ...
  const output = (outputVal: any, center: boolean) => {
  ...
  const onparsed = (cmd: any, ...args: any[]) => {
  ...
  const kbd: any = keyboard(parse)
  const clear = () => {
    $element.value = ''
    return ''
  }
  const input = (ev: any) => (busy ? ev.preventDefault() : kbd[ev.type](ev))
