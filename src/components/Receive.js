...
const AddressBlock = ({
  ...
  return <AccordionItem key={copied} className={[cstyles.well, styles.receiveblock].join(' ')} uuid={address}>
      ...
      <AccordionItemPanel className={[styles.receiveDetail].join(' ')}>
        <div className={[cstyles.flexspacebetween].join(' ')}>
          <div className={[cstyles.verticalflex, cstyles.marginleft].join(' ')}>
            {label && <div className={cstyles.margintoplarge}>
            ...
            <div className={[cstyles.margintoplarge, cstyles.breakword].join(' ')}>
              {privateKey && <div>
                  ...
                  <div className={[cstyles.breakword, cstyles.padtopsmall, cstyles.fixedfont].join(' ')} style={{
                maxWidth: '600px'
              }}>
                    {privateKey}
                  </div>
            ...
            <div>
              <button className={[cstyles.primarybutton, cstyles.margintoplarge].join(' ')} type="button" onClick={() => {
              ...
              {!privateKey && <button className={[cstyles.primarybutton].join(' ')} type="button" onClick={() => fetchAndSetSinglePrivKey(address)}>
                  Export Private Key
                </button>}

              {Utils.isZaddr(address) && !viewKey && <button className={[cstyles.primarybutton].join(' ')} type="button" onClick={() => fetchAndSetSingleViewKey(address)}>
                  Export Viewing Key
                </button>}

              {Utils.isTransparent(address) && <button className={[cstyles.primarybutton].join(' ')} type="button" onClick={() => openAddress()}>
...
export default class Receive extends Component {
  render() {
    ...
    const zaddrs = addresses.filter(a => Utils.isSapling(a)).slice(0, 100).map(a => new AddressBalance(a, addressMap[a]));
    ...
    const taddrs = addresses.filter(a => Utils.isTransparent(a)).slice(0, 100).map(a => new AddressBalance(a, addressMap[a]));
            ...
            <TabPanel key={`z${rerenderKey}`}>
              ...
              <ScrollPane offsetHeight={100}>
                <Accordion preExpanded={[defaultZaddr]}>
                  {zaddrs.map(a => <AddressBlock key={a.address} addressBalance={a} currencyName={info.currencyName} label={addressBookMap[a.address]} pslPrice={info.pslPrice} privateKey={addressPrivateKeys[a.address]} viewKey={addressViewKeys[a.address]} fetchAndSetSinglePrivKey={fetchAndSetSinglePrivKey} fetchAndSetSingleViewKey={fetchAndSetSingleViewKey} rerender={this.rerender} />)}
                </Accordion>

                <button className={[cstyles.primarybutton, cstyles.margintoplarge, cstyles.marginbottomlarge].join(' ')} onClick={() => createNewAddress(true)} type="button">
            ...
            <TabPanel key={`t${rerenderKey}`}>
              ...
              <ScrollPane offsetHeight={100}>
                <Accordion preExpanded={[defaultTaddr]}>
                  {taddrs.map(a => <AddressBlock key={a.address} addressBalance={a} currencyName={info.currencyName} pslPrice={info.pslPrice} privateKey={addressPrivateKeys[a.address]} viewKey={addressViewKeys[a.address]} fetchAndSetSinglePrivKey={fetchAndSetSinglePrivKey} fetchAndSetSingleViewKey={fetchAndSetSingleViewKey} rerender={this.rerender} />)}
                </Accordion>

                <button className={[cstyles.primarybutton, cstyles.margintoplarge, cstyles.marginbottomlarge].join(' ')} type="button" onClick={() => createNewAddress(false)}>
