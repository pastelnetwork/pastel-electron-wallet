  ...
  render() {
    ...
    return <>
        ...
        <div className={styles.container}>
            ...
            <div className={cstyles.well}>
              <div className={cstyles.flexspacebetween}>
                ...
                <div className={cstyles.validationerror}>
                  {passphrase && <span className={passphraseStatusColor(passphraseValidation)}>{passphraseValidation.value}</span>}
                </div>
              </div>

              <input type="text" className={[cstyles.inputbox, cstyles.margintopsmall].join(' ')} onChange={this.onPassphraseChange.bind(this)} placeholder="Passphrase" />
              ...
              <div className={cstyles.margintoplarge}>
                <button type="button" disabled={!this.valid} className={[cstyles.primarybutton, cstyles.margintoplarge, styles.button].join(' ')} onClick={this.onCreate.bind(this)}>
                  Create
                </button>
              </div>
            </div>
          ...
          {pastelIDs.length > 0 && <List title="Pastel ID">
              {pastelIDs.map(item => <ListItem key={uid()} title={item.pastelid} />)}
            </List>}

          {pastelIDs.length === 0 && <div className={cstyles.margintoplarge}>There are currently no Pastel IDs generated.</div>}
        </div>
      </>;
  }
