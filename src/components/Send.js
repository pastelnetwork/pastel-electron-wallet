...
const ToAddrBox = ({
  ...
  if (!addressIsValid || amountError || !memoIsValid || toaddr.to === '' || parseFloat(toaddr.amount) === 0 || fromAmount === 0) {
    buttonstate = false;
  } else {
    buttonstate = true;
  }
      ...
      <div className={[cstyles.well, cstyles.verticalflex].join(' ')}>
        <div className={[cstyles.flexspacebetween].join(' ')}>
          ...
          <div className={cstyles.validationerror}>
            {addressIsValid ? <i className={[cstyles.green, 'fas', 'fa-check'].join(' ')} /> : <span className={cstyles.red}>Invalid Address</span>}
          </div>
        </div>
        <input type="text" placeholder="Z or T address" className={cstyles.inputbox} value={toaddr.to} onChange={e => updateToField(toaddr.id, e, null, null)} />
        <Spacer />
        <div className={[cstyles.flexspacebetween].join(' ')}>
        ...
        <div className={[cstyles.flexspacebetween].join(' ')}>
          <input type="number" step="any" className={cstyles.inputbox} value={isNaN(toaddr.amount) ? '' : toaddr.amount} onChange={e => updateToField(toaddr.id, null, e, null)} />
          <img className={styles.toaddrbutton} src={ArrowUpLight} alt="Max" onClick={() => setMaxAmount(toaddr.id, totalAmountAvailable)} />
        </div>
        ...
        {!isMemoDisabled && <div>
            ...
            <TextareaAutosize className={cstyles.inputbox} value={toaddr.memo} disabled={isMemoDisabled} onChange={e => updateToField(toaddr.id, null, null, e)} />
...
function getSendManyJSON(sendPageState, info) {
  ...
  json.push(sendPageState.toaddrs.map(to => {
...
const ConfirmModalToAddr = ({
  ...
  const {
    bigPart,
    smallPart
  } = Utils.splitPslAmountIntoBigSmall(toaddr.amount);
  const memo = toaddr.memo ? toaddr.memo : '';
  return <div className={cstyles.well}>
...
const ConfirmModalInternal = ({
  ...
  const sendingTotal = sendPageState.toaddrs.reduce((s, t) => parseFloat(s) + parseFloat(t.amount), 0.0) + Utils.getDefaultFee(info.latestBlock);
  const {
    bigPart,
    smallPart
  } = Utils.splitPslAmountIntoBigSmall(sendingTotal);
  ...
  const sendButton = () => {
    ...
    openErrorModal('Computing Transaction', 'Please wait...This could take a while'); // Then send the Tx async
  ...
  return <Modal isOpen={modalIsOpen} onRequestClose={closeModal} className={styles.confirmModal} overlayClassName={styles.confirmOverlay}>
      <div className={[cstyles.verticalflex].join(' ')}>
        <div className={[cstyles.marginbottomlarge, cstyles.center].join(' ')}>Confirm Transaction</div>
        <div className={cstyles.flex}>
          <div className={[cstyles.highlight, cstyles.xlarge, cstyles.flexspacebetween, cstyles.well, cstyles.maxwidth].join(' ')}>
            <div>Total</div>
        ...
        <div className={[cstyles.verticalflex, cstyles.margintoplarge].join(' ')}>
          {sendPageState.toaddrs.map(t => <ConfirmModalToAddr key={t.to} toaddr={t} info={info} />)}
        </div>

        <ConfirmModalToAddr toaddr={{
        to: 'Fee',
        amount: Utils.getDefaultFee(info.latestBlock),
        memo: null
      }} info={info} />

        {info && info.disconnected && <div className={[cstyles.red, cstyles.margintoplarge].join(' ')}>
            You are currently disconnected. This transaction might not work.
          </div>}

        {info && info.verificationprogress < 0.99 && <div className={[cstyles.red, cstyles.margintoplarge].join(' ')}>
...
export default class Send extends PureComponent {
  ...
  addToAddr = () => {
    ...
    const newToAddrs = sendPageState.toaddrs.concat(new ToAddr(Utils.getNextToAddrID())); // Create the new state object
  ...
  changeFrom = selectedOption => {
    const {
      sendPageState,
      setSendPageState
    } = this.props; // Create the new state object
  ...
  setMaxAmount = (id, total) => {
    ...
    let totalOtherAmount = newToAddrs.filter(a => a.id !== id).reduce((s, a) => parseFloat(s) + parseFloat(a.amount), 0); // Add Fee
    ...
    return <div>
        ...
        <div className={styles.sendcontainer}>
          <div className={[cstyles.well, cstyles.verticalflex].join(' ')}>
            ...
            <Select value={fromaddr} options={sendFromList} styles={customStyles} // $FlowFixMe
          ...
          <ScrollPane offsetHeight={300}>
            ...
            return <ToAddrBox key={toaddr.id} toaddr={toaddr} pslPrice={info.pslPrice} updateToField={this.updateToField} fromAddress={fromaddr.value} fromAmount={totalAmountAvailable} setMaxAmount={this.setMaxAmount} setSendButtonEnable={this.setSendButtonEnable} totalAmountAvailable={totalAmountAvailable} />;
          })}
            <div style={{
            textAlign: 'right'
          }}>
          ...
          <div className={cstyles.center}>
            <button type="button" disabled={!sendButtonEnabled} className={cstyles.primarybutton} onClick={this.openModal}>
              Send
            </button>
          ...
          <ConfirmModal sendPageState={sendPageState} info={info} sendTransaction={sendTransaction} openErrorModal={openErrorModal} closeModal={this.closeModal} modalIsOpen={modalIsOpen} clearToAddrs={this.clearToAddrs} />

          <ErrorModal title={errorModalTitle} body={errorModalBody} modalIsOpen={errorModalIsOpen} closeModal={closeErrorModal} />
        </div>
      </div>;
  }
