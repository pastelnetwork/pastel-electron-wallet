...
export default class CompanionAppListener {
  ...
  processIncoming(data, keyHex, ws) {
    ...
    if (this.tmpWormholeClient && keyHex === this.tmpWormholeClient.getKeyHex()) {
      const {
        decrypted,
        nonce
      } = this.decryptIncoming(data, keyHex, false);
      ...
      const {
        decrypted,
        nonce
      } = this.decryptIncoming(data, keyHex, true);
  ...
  getLocalNonce() {
    // Get the nonce. Increment and store the nonce for next use
    const store = new Store();
    const nonceHex = store.get('companion/localnonce', `01${'00'.repeat(this.sodium.crypto_secretbox_NONCEBYTES - 1)}`); // Increment nonce
